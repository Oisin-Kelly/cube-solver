...
std::unordered_set<Face> CrossSolver::getAlignedFaces()
{
    ...
    if (cube->getCube()[7] == UP && cube->getCube()[10] == FRONT)
        alignedFaces.insert(FRONT);
    if (cube->getCube()[5] == UP && cube->getCube()[19] == RIGHT)
        alignedFaces.insert(RIGHT);
    if (cube->getCube()[1] == UP && cube->getCube()[28] == BACK)
        alignedFaces.insert(BACK);
    if (cube->getCube()[3] == UP && cube->getCube()[37] == LEFT)
...
void CrossSolver::solve()
{
    ...
    while (alignedFaces.size() != 4)
    {
        ...
        {
            if (cube->getCube()[edge.first] != UP)
                continue;

            const Face c_edge = cube->getCube()[edge.second]; // edge number for colored side of edge; 1 .. 4
            ...
            const uint8_t l_index = edge.second % 9;

            if (N == -1 || N == 1)
            {
                if (Helpers::get_row(l_index) == 0)
                {
                    cube->rotateSide(c_side, -N);
                    alignEdge(c_edge, N);
                }
                else if (Helpers::get_row(l_index) == 1)
                {
                    if (N == 1 && Helpers::get_column(l_index) == 2)
                    {
                    ...
                    else if (N == -1 && Helpers::get_column(l_index) == 0)
                    {
                        cube->rotateSide(c_side, 2);
                        cube->rotateSide(Face::UP);
            ...
            else if (N == 2)
            {
                ...
                {
                    const int sideRotation = Helpers::get_column(l_index) == 0 ? -1 : 1;
